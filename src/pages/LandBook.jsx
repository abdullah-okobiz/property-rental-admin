...
const LandBook = () => {
  ...
  const queryKey = ["flatBookings", { page }];
  ...
  const columns = [
    ...
    {
      title: "Flat Title",
      render: (_, record) => record.flat?.title || "N/A",
      key: "title",
    },
    {
      title: "Flat Price",
      render: (_, record) => `BDT ${record.flat?.price || "N/A"}`,
      key: "price",
    },
    {
      title: "Category",
      render: (_, record) => record.flat?.category || "N/A",
      key: "category",
    },
    {
      title: "Host",
      render: (_, record) => (
        <div className="text-sm space-y-1">
          <div className="flex justify-between">
            ...
            <span>{record.flat?.host?.name || "N/A"}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500 font-medium">Email:</span>
            <span>{record.flat?.host?.email || "N/A"}</span>
          </div>
        </div>
      ),
      key: "host",
    ...
    },
    {
      title: "Cover Image",
      render: (_, record) =>
        record.flat?.coverImage ? (
          <Image width={80} src={`${baseUrl}${record.flat.coverImage}`} />
        ) : (
